 ...
 */
static void
vxlan_fwd_build_pipe_common_match_fields(struct doca_flow_match *match)
{
 ...
 */
static int
vxlan_fwd_create_match_pipe(struct vxlan_fwd_port_cfg *port_cfg, enum doca_flow_tun_type type)
{
	struct doca_flow_match match;
	struct doca_flow_actions actions, *actions_arr[NB_ACTION_ARRAY];
	...
	case DOCA_FLOW_TUN_VXLAN:
		...
		pipe = &vxlan_fwd_ins->pipe_vxlan[port_cfg->port_id];
		break;
	default:
		return -1;
	}

	vxlan_fwd_build_pipe_common_match_fields(&match);
 ...
 */
static int
vxlan_fwd_create_control_pipe(struct vxlan_fwd_port_cfg *port_cfg)
{
	struct doca_flow_pipe_cfg pipe_cfg;
	...
	if (doca_flow_pipe_create(&pipe_cfg, NULL, NULL, &vxlan_fwd_ins->pipe_control[port_cfg->port_id]) != DOCA_SUCCESS)
		return -1;
	return 0;
}
 ...
 */
static int
vxlan_fwd_add_control_pipe_entries(struct vxlan_fwd_port_cfg *port_cfg)
{
	struct doca_flow_match match;
	struct doca_flow_fwd fwd;
	struct doca_flow_pipe_entry *entry;
	...
	fwd.type = DOCA_FLOW_FWD_PIPE;
	fwd.next_pipe = vxlan_fwd_ins->pipe_vxlan[port_cfg->port_id];

	result = doca_flow_pipe_control_add_entry(0, priority, vxlan_fwd_ins->pipe_control[port_cfg->port_id], &match,
	...
	priority = 1;
	fwd.type = DOCA_FLOW_FWD_PIPE;
	fwd.next_pipe = vxlan_fwd_ins->pipe_hairpin[port_cfg->port_id];
 ...
 */
static doca_error_t
vxlan_fwd_create_vxlan_encap_pipe(struct vxlan_fwd_port_cfg *port_cfg)
{
	struct doca_flow_match match;
	...
	return doca_flow_pipe_create(&pipe_cfg, &fwd, NULL, &vxlan_fwd_ins->vxlan_encap_pipe[port_cfg->port_id]);
}
 ...
 */
static doca_error_t
vxlan_fwd_add_vxlan_encap_pipe_entry(struct vxlan_fwd_port_cfg *port_cfg)
{
	struct doca_flow_match match;
	...
	uint8_t src_mac[] = {0xde,0xed,0xbe,0xef,0xab,0xcd};
	...
	result = doca_flow_pipe_add_entry(0, vxlan_fwd_ins->vxlan_encap_pipe[port_cfg->port_id], &match, &actions, NULL, NULL, 0, status, &entry);
	if (result != DOCA_SUCCESS) {
		free(status);
		return -1;
	}
...
vxlan_fwd_init_ports_and_pipes(struct vxlan_fwd_port_cfg *port_cfg)
{
	...
	for (port_id = 0; port_id < nb_ports; port_id++) {
		...
		DOCA_LOG_INFO("Create VXLAN match pipe...");
		result = vxlan_fwd_create_match_pipe(curr_port_cfg, DOCA_FLOW_TUN_VXLAN);
		if (result < 0) {
			DOCA_LOG_ERR("Failed building VXLAN pipe");
			return -1;
		}

		DOCA_LOG_INFO("Create control pipe...");
		result = vxlan_fwd_create_control_pipe(curr_port_cfg);
		if (result < 0) {
			DOCA_LOG_ERR("Failed building control pipe");
			return -1;
		}

		DOCA_LOG_INFO("Add control pipe entries...");
		result = vxlan_fwd_add_control_pipe_entries(curr_port_cfg);
		if (result < 0) {
			DOCA_LOG_ERR("Failed adding entries to the control pipe");
			return -1;
		}

		DOCA_LOG_INFO("Create VXLAN encap pipe...");
