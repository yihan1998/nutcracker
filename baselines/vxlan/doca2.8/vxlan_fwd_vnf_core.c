...
DOCA_LOG_REGISTER(VXLAN_FWD_VNF:Core);
 ...
 */
static void
vxlan_fwd_process_offload(struct rte_mbuf *mbuf, uint16_t queue_id, struct app_vnf *vnf)
{
	struct vxlan_fwd_pkt_info pinfo;

	memset(&pinfo, 0, sizeof(struct vxlan_fwd_pkt_info));
	if (vxlan_fwd_parse_packet(VNF_PKT_L2(mbuf),
...
int
vxlan_fwd_process_pkts(void *process_pkts_params)
{
	int result;
	...
	struct vxlan_fwd_config *app_config = ((struct vxlan_fwd_process_pkts_params *) process_pkts_params)->cfg;
	struct app_vnf *vnf = ((struct vxlan_fwd_process_pkts_params *) process_pkts_params)->vnf;
	...
	while (!force_quit) {
		...
		for (port_id = 0; port_id < NUM_OF_PORTS; port_id++) {
			...
			if (nb_rx) printf("Receive %d packets\n", nb_rx);
			for (j = 0; j < nb_rx; j++) {
				if (app_config->hw_offload)
					vxlan_fwd_process_offload(mbufs[j], queue_id, vnf);
...
void
vxlan_fwd_process_pkts_stop(void)
{
	force_quit = true;
}
 ...
 */
doca_error_t
register_vxlan_fwd_params(void)
{
	doca_error_t result;
...
void
vxlan_fwd_map_queue(uint16_t nb_queues)
{
	int i, queue_idx = 0;
...
void
vxlan_fwd_destroy(struct app_vnf *vnf)
{
	vnf->vnf_destroy();
}
