...
#include "backends/doca/docaProgram.h"

void emitDocaProgram(const DocaOptions &options, Pipeline& pipeline, Target *target) {
    CallGraph callGraph = pipeline.callGraph;
    CodeBuilder c(target);
    CodeBuilder h(target);

    auto docaprog = new DocaProgram(callGraph);
...
void run_doca_backend(const DocaOptions &options, Pipeline& pipeline) {
    std::cout << "\033[42m\033[37m" << "Running DOCA backend for pipeline " << pipeline.name << "\033[0m" << std::endl;
    auto target = new DocaTarget();
    emitDocaProgram(options, pipeline, target);
    return;
}
