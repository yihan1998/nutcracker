...
#include "backends/c/cBackend.h"

using json = nlohmann::json;

#define CONFIG_FSM_BACKEND  1
...
int main(int argc, char ** argv) {
...
#elif CONFIG_FSM_BACKEND
    auto options = COptions(argc, argv);
    if (!options.inputFile.empty()) {
        std::cout << "Input file: " << options.inputFile << std::endl;
    }
    ...
    std::cout << "\033[42m\033[37m" << "Generating C backend" << "\033[0m" << std::endl;
    for (auto pipeline : pipelines) {
        auto options = COptions(argc, argv);
        if (!options.inputFile.empty()) {
            std::cout << "Input file: " << options.inputFile << std::endl;
        }
        run_c_backend(options, pipeline);
    }
    std::cout << "[DONE]" << std::endl;
#endif
    return 0;
