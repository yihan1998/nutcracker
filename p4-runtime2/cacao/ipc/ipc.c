...
struct list_head attached_procs;
...
int create_msg_qp(const char *name) {
    char qp_name[64] = {0};
    struct attached_proc * new_proc;

    new_proc = (struct attached_proc *)calloc(1, sizeof(struct attached_proc));

    sprintf(qp_name, "%s_rq", name);
    new_proc->recv_queue = rte_ring_create(qp_name, 2048, rte_socket_id(), 0);
    assert(new_proc->recv_queue != NULL);

    sprintf(qp_name, "%s_sq", name);
    new_proc->send_queue = rte_ring_create(qp_name, 2048, rte_socket_id(), 0);

    list_add_tail(&new_proc->list, &attached_procs);
...
static int handle_epollin_event(int epfd, struct epoll_event * ev) {
    ...
    if (parent == control_fd) {
        ...
        if (strcmp(buffer, "CONNECT_PROCESS") == 0) {
            create_msg_qp("test");
            if (send(sockfd, reply, sizeof(reply), 0) == -1) {
