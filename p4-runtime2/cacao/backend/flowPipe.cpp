    ...
    for (auto entry : tablePipe->entries) {
        if (fsm_table_lookup_internal(&tablePipe->match, &entry.match, skb)) {
            ...
            if (memcmp(&entry.action, &zeroAction, sizeof(struct flow_actions))!=0) {
                ...
                // switch (argList.size()) {
                //     case 7:
                //         pipe->swPipe.ops.run(pipe, skb, argList[0], argList[1], argList[2], argList[3], argList[4], argList[5], argList[6]);
                //         break;
...
extern "C" int fsm_table_add_entry(struct flow_pipe* pipe, struct flow_match* match, struct flow_actions* action, struct flow_fwd* fwd) {
    // std::vector<ArgBase*> argList;
    // if (match->outer.udp.dest) {
    //     ArgUInt16 * udp_dest = new ArgUInt16(match->outer.udp.dest);
    //     argList.push_back(udp_dest);
    // }
