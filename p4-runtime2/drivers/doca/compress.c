...
doca_error_t docadv_compress_init(void) {
	...
	if (result != DOCA_SUCCESS) {
#ifdef CONFIG_BLUEFIELD2
		pr_err("DOCA Compress creation Failed. Reason: %s", doca_get_error_string(result));
#elif CONFIG_BLUEFIELD3
		pr_err("DOCA Compress creation Failed. Reason: %s", doca_error_get_descr(result));
#endif
	...
	if (result != DOCA_SUCCESS) {
#ifdef CONFIG_BLUEFIELD2
		pr_err("Unable to install Compress device. Reason: %s", doca_get_error_string(result));
#elif CONFIG_BLUEFIELD3
		pr_err("Unable to install Compress device. Reason: %s", doca_error_get_descr(result));
#endif
...
int docadv_deflate(z_streamp strm, int flush) {
    doca_error_t res;
	struct docadv_compress_ctx * compress_ctx = &lcore_ctx->docadv_ctx.compress_ctx;
	...
	res = doca_workq_submit(lcore_ctx->docadv_ctx.workq, &compress_job.base);
	if (res != DOCA_SUCCESS) {
    ...
    do {
		res = doca_workq_progress_retrieve(lcore_ctx->docadv_ctx.workq, &event, DOCA_WORKQ_RETRIEVE_FLAGS_NONE);
	    if (res != DOCA_SUCCESS) {
